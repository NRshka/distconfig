...
class ConfigHandler:
    def __init__(self, parsers: Dict[str, AbstractConfigParser], cfg_dir: Optional[str] = None, configs: Optional[Dict[str, dict]] = None):
        assert cfg_dir or configs

        self.parsers = parsers
        self.cfg_dir = cfg_dir
        self.configs: Dict[str, Dict[str, dict]] = {}  # service name : config name: config
        ...
        for project_name in service_names:
            ...
            for config_name in config_files:
                self.configs[project_name][config_name] = self.parse_config_file(os.path.join(project_path, config_name))
    ...
    def select_suitable_parser(self, path):
        ...
        if extension not in self.parsers:
            raise ValueError(f'No suitable parse for extension {extension} of path {path}')
